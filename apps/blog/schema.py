...
from .security import (
    ...
    audit_operation, validate_input, BlogRoles, FieldAccessLevel
)
...
class CreatePost(graphene.Mutation):
    ...
    def mutate(root, info, input):
        ...
        user = info.context.user
        if user.is_anonymous:
            return CreatePost(post=None, success=False, errors=["Authentication required"])
        ...
        try:
            ...
            if not input.get('title') or not input.get('title').strip():
                return CreatePost(post=None, success=False, errors=['Title is required'])
...
class CreateComment(graphene.Mutation):
    ...
    def mutate(root, info, post_id, content, parent_id=None):
        ...
        user = info.context.user
        if user.is_anonymous:
            return CreateComment(comment=None, success=False, errors=["Authentication required"])
        
        # Validate content
        if not content or not content.strip():
            return CreateComment(comment=None, success=False, errors=["Content is required"])
...
class SubscribeNewsletter(graphene.Mutation):
    ...
    def mutate(root, info, email, name=None):
        ...
        if not email or not email.strip():
            return SubscribeNewsletter(subscriber=None, success=False, errors=["Email is required"])
